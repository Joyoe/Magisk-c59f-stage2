...
static void patch_init_rc(const char *src, const char *dest, const char *tmp_dir) {
    ...
    for (auto &script : rc_list) {
        ...
        replace_all(script, "${JOKERTMP}", tmp_dir);
        fprintf(rc, "\n%s\n", script.data());
    }
    rc_list.clear();
    ...
    fprintf(rc, JOKER_RC, tmp_dir, pfd_svc, ls_svc, bc_svc);
...
bool JoiskInit::patch_sepolicy(const char *file) {
...
void SARBase::patch_rootdir() {
    ...
    // Extract magisk
    {
        auto magisk = mmap_data::ro("joisk32.xz");
        unlink("joisk32.xz");
        int fd = xopen("joisk32", O_WRONLY | O_CREAT, 0755);
        unxz(fd, magisk.buf, magisk.sz);
        close(fd);
        patch_socket_name("joisk32");
        if (access("joisk64.xz", F_OK) == 0) {
            magisk = mmap_data::ro("joisk64.xz");
            unlink("joisk64.xz");
            fd = xopen("joisk64", O_WRONLY | O_CREAT, 0755);
            unxz(fd, magisk.buf, magisk.sz);
            close(fd);
            patch_socket_name("joisk64");
            xsymlink("./joisk64", "magisk");
        } else {
            xsymlink("./joisk32", "magisk");
        }
    }
...
void MagiskProxy::start() {
    ...
    auto magisk = mmap_data::ro("/sbin/joisk32.xz");
    auto joisk64 = mmap_data::ro("/sbin/joisk64.xz");
    char custom_rules_dir[64];
    custom_rules_dir[0] = '\0';
    xreadlink(TMP_RULESDIR, custom_rules_dir, sizeof(custom_rules_dir));

    unlink("/sbin/magisk");
    unlink("/sbin/joisk32.xz");
    unlink("/sbin/joisk64.xz");
    rm_rf("/.backup");

    setup_tmp("/sbin");

    // Extract magisk
    int fd = xopen("/sbin/joisk32", O_WRONLY | O_CREAT, 0755);
    unxz(fd, magisk.buf, magisk.sz);
    close(fd);
    patch_socket_name("/sbin/joisk32");
    if (joisk64.sz) {
        fd = xopen("/sbin/joisk64", O_WRONLY | O_CREAT, 0755);
        unxz(fd, joisk64.buf, joisk64.sz);
        close(fd);
        patch_socket_name("/sbin/joisk64");
        xsymlink("./joisk64", "/sbin/magisk");
    } else {
        xsymlink("./joisk32", "/sbin/magisk");
    }
