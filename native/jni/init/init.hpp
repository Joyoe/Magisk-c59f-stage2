...
class JoiskInit : public BaseInit {
protected:
    mmap_data self;
    mmap_data config;
...
public:
    JoiskInit(char *argv[], cmdline *cmd) : BaseInit(argv, cmd) {}
};

class SARBase : public JoiskInit {
protected:
...
public:
    SARBase(char *argv[], cmdline *cmd) : JoiskInit(argv, cmd) {}
};
...
class RootFSInit : public JoiskInit {
private:
    void early_mount();
    void patch_rootfs();
public:
    RootFSInit(char *argv[], cmdline *cmd) : JoiskInit(argv, cmd) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override {
        early_mount();
...
class MagiskProxy : public JoiskInit {
public:
    explicit MagiskProxy(char *argv[]) : JoiskInit(argv, nullptr) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override;
};
