...
static void handle_request(int client) {
    ...
    case REMOVE_MODULES:
        ...
        break;
    case JOKERHIDE:  // accept hide request from zygote
    ...
    switch (req_code) {
    case CHECK_VERSION:
        write_string(client, JOKER_VERSION ":JOKER");
        goto shortcut;
    case CHECK_VERSION_CODE:
        write_int(client, JOKER_VER_CODE);
        goto shortcut;
    case GET_PATH:
        write_string(client, JOKERTMP.data());
        goto shortcut;
    case START_DAEMON:
        setup_logfile(true);
        goto shortcut;
...
[[noreturn]] static void daemon_entry() {
    ...
    // SAR cleanups
    auto mount_list = JOKERTMP + "/" ROOTMNT;
    ...
    // Load config status
    auto config = JOKERTMP + "/" INTLROOT "/config";
