    ...
    void mount() override {
        ...
        if (name() == "magisk") {
            ...
            for (int i = 0; init_applet[i]; ++i) {
                ...
                VLOGD("create", "./joiskinit", dest.data());
                xsymlink("./joiskinit", dest.data());
            }
        }
        create_and_mount(JOKERTMP + "/" + name());
    }
};
...
static void inject_magisk_bins(root_node *system) {
    ...
    bin->insert(new magisk_node("joiskinit"));
...
void magic_mount() {
    node_entry::mirror_dir = JOKERTMP + "/" MIRRDIR;
    node_entry::module_mnt = JOKERTMP + "/" MODULEMNT "/";
    ...
    for (const auto &m : module_list) {
        auto module = m.data();
        char *b = buf + sprintf(buf, "%s/" MODULEMNT "/%s/", JOKERTMP.data(), module);
...
static void prepare_modules() {
    ...
    auto src = JOKERTMP + "/" MIRRDIR MODULEROOT;
    auto dest = JOKERTMP + "/" MODULEMNT;
