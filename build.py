...
default_targets = ['magisk', 'joiskinit', 'magiskboot', 'busybox']
...
def dump_bin_headers():
    ...
    if not op.exists(stub):
        error('Build stub APK before building "joiskinit"')
...
def build_binary(args):
    ...
    # Basic flags
    global base_flags
    base_flags = f'JOKER_VERSION={config["version"]} JOKER_VER_CODE={config["versionCode"]}'
    if not args.release:
        base_flags += ' JOKER_DEBUG=1'
