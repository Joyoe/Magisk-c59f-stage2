...
env_check() {
  for file in busybox magiskboot joiskinit util_functions.sh boot_patch.sh; do
    [ -f $JOKERBIN/$file ] || return 1
  done
  return 0
}
...
fix_env() {
  ...
  chmod 700 $NVBASE
  cp_readlink $1 $JOKERBIN
  rm -rf $1
  chown -R 0:0 $JOKERBIN
}
...
add_hosts_module() {
  ...
  [ -d $JOKERTMP/modules/hosts ] && return
  cd $JOKERTMP/modules
...
app_init() {
  ...
  get_flags
  run_migrations
  SHA1=$(grep_prop SHA1 $JOKERTMP/config)
