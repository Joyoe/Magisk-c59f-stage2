...
class BusyBoxInit : BaseShellInit() {
    ...
    override fun init(context: Context, shell: Shell): Boolean {
        shell.newJob().apply {
            ...
            if (shell.isRoot) {
                add("export JOKERTMP=\$(magisk --path)/.magisk")
            ...
            if (Info.noDataExec) {
                ...
                add(
                    "if [ -x \$JOKERTMP/busybox/busybox ]; then",
                    "  cp -af $localBB \$JOKERTMP/busybox/busybox",
                    "  exec \$JOKERTMP/busybox/busybox sh",
                    "else",
...
class AppShellInit : BaseShellInit() {
    ...
    override fun init(context: Context, shell: Shell): Boolean {
        ...
        Const.JOKERTMP = getVar("JOKERTMP")
