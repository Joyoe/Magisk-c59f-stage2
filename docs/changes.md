...
- [sepolicy-inject] Complete re-write the command-line options, now nearly matches jojopolicy syntax
...
- Fully open source
- Remove jojopolicy dependency, use my own sepolicy-injection
...
- Include updated open source sepolicy injection tool (source code available), support nearly all SuperSU jojopolicy tool's functionality

### v3

- Fix bootimg-extract for Exynos Samsung devices (thanks to @phhusson), should fix all Samsung device issues
- Add jojopolicy back to patch sepolicy (stock Samsung do not accept permissive domain)
...
- Remove sepolicy patches that uses SuperSU's jojopolicy tool; it is now using a minimal set of modifications
